...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self.add_box_patch = cfg.VIT.ADD_BOX_PATCH
        self.cfg = cfg
        ...
        if self.cfg.VIT.ATTN_LAYER == "divided":
            ...
            self.blocks = nn.ModuleList([
                vit_helper.Block(
                    ...
                    add_box_patch = self.add_box_patch,
    ...
    def forward_features(self, x, boxes):
        if self.video_input and self.cfg.MODEL.MODEL_NAME=='VisionTransformer':
            x = x[0]
        B = x.shape[0]

        # Tokenize input
        x = self.patch_embed_3d(x) # B,THW,C
        ...
        for i, blk in enumerate(self.blocks):
            ...
            if i + 1 in self.cfg.VIT.LAYERS_OUT:
                out.append(self.norm(x))

        out = torch.stack(out,1)
        if self.cfg.MODEL.MODEL_NAME=='VisionTransformer':
            x = self.norm(x)
            cls_x = self.pre_logits(x[:, 0])
        else:
            # no pre_logits, just take the cls token
            cls_x = self.norm(x[:, 0])

        return cls_x, out

    def forward(self, x, meta, return_features=False, features_in=False):
        boxes = meta['boxes']
        cls_x, out = self.forward_features(x,boxes)
        cls_x = self.head_drop(cls_x)
        if self.cfg.MODEL.MODEL_NAME=='VisionTransformer':
            if isinstance(self.num_classes, (list,)) and len(self.num_classes) > 1:
                output = []
                for head in range(len(self.num_classes)):
                    x_out = getattr(self, "head%d"%head)(cls_x)
        ...
        if not return_features:
            return cls_x, cls_x, None, None, None 
        else:
            return cls_x, out, None, None, None
